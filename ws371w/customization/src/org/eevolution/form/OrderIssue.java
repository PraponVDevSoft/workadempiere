 ...
 */
/*
 * prapon 2019/11/29 , only issue for users in process line
 */
public class OrderIssue extends GenForm {

	/** Logger */
	private static CLogger log = CLogger.getCLogger(OrderIssue.class);
	String m_sql = "";

	private boolean m_isOnlyReceipt = false;

	private boolean m_OnlyIssue = true; // false prapon 2019/11/29  ?? uneffect
	...
	public void executeQuery(IMiniTable issue) {
		//Integer lo = (Integer) locatorField.getValue();   //  add, prapon 2020/01/10
		final String sql = "SELECT "
				...
				+ "bomQtyAvailable(obl.M_Product_ID,obl.M_Warehouse_ID,0) AS QtyAvailable," // 10
				+ "bomQtyOnHand(obl.M_Product_ID,obl.M_Warehouse_ID,0) AS QtyOnHand," // 11
				+ "obl.M_Locator_ID," // 12  ,old p.-- Aon 07/12/2015
				+ "obl.M_Warehouse_ID,w.Name," // 13,14
				+ "obl.QtyBom," // 15
				+ "obl.isQtyPercentage," // 16
				+ "obl.QtyBatch," // 17
				...
				+ " INNER JOIN C_UOM u ON (p.C_UOM_ID = u.C_UOM_ID) "
				+ " INNER JOIN M_Warehouse w ON (w.M_Warehouse_ID = obl.M_Warehouse_ID) "
				+ " WHERE obl.PP_Order_ID = ? and obl.M_Locator_ID = " + m_M_Locator_ID  
